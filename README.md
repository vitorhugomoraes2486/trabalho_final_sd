...
7. Fluxos Principais do Sistema
   ...
   7.1 **Inicialização e DNS:** O Cliente faz uma resolução de nomes via DNS local para localizar o endereço lógico `mestre-crack.local` e envia o hash de destino.
   
   7.2 **Carga e Replicação Física:** O Mestre recebe o hash e insere 26 tarefas (uma para cada letra inicial do alfabeto de 'a' a 'z') com o status de `disponivel` no Master DB. O mecanismo interno do PostgreSQL replica instantaneamente essas linhas para o Slave DB.
   
   7.3 **Consumo Assíncrono (Read):** Os Workers consultam continuamente o Slave DB (Porta 5433) em busca de lotes livres.
   
   7.4 **Reserva Segura (Write):** Ao escolher um lote, o Worker envia um `POST` ao Mestre. O Mestre aplica o `UPDATE` no Master DB apenas se a linha ainda estiver livre, propagando a trava pela replicação e impedindo concorrência.
   
   7.5 **Término Distribuído:** O Worker executa a força bruta localmente. Se encontrar o resultado, notifica o endpoint `/api/sucesso`. Os demais nós percebem a mudança de estado global nas suas checagens periódicas e abortam a execução.


9. Características de Sistemas Distribuídos Implementadas
